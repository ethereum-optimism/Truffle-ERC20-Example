...
contract('ERC20', (accounts) => {
  ...
  beforeEach(async () => {
    token = await ERC20.new(10000, tokenName, tokenDecimals, tokenSymbol, {
      from: accounts[ 0 ],
      gasPrice: 0
    })
  })
  ...
  it('transfers: should handle zero-transfers normally', async () => {
    assert(await token.transfer.call(accounts[ 1 ], 0, {
      from: accounts[ 0 ],
      gasPrice: 0
  ...
  it('approvals: approve max (2^256 - 1)', async () => {
    await token.approve(accounts[ 1 ], '115792089237316195423570985008687907853269984665640564039457584007913129639935', {
      from: accounts[ 0 ],
      gasPrice: 0
    })
  ...
  it('approvals: msg.sender approves accounts[1] of max (2^256 - 1) & withdraws 20', async () => {
    ...
    await token.approve(accounts[ 1 ], max, {
      from: accounts[ 0 ],
      gasPrice: 0
    })
    const balance2 = await token.balanceOf.call(accounts[ 2 ])
  ...
  it('events: should fire Transfer event normally on a zero transfer', async () => {
    const res = await token.transfer(accounts[ 1 ], '0', {
      from: accounts[ 0 ],
      gasPrice: 0
    })
    const transferLog = res.logs.find(
